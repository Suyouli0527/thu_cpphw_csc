...
classDiagram
    ...
    BankSystem *-- DateManager : 组合
    BankSystem *-- LogManager : 组合
    BankSystem *-- TransactionManager : 组合

    AccountManager *-- Account : 组合（拥有所有权）
    ...
    UserManager ..> AccountManager : 依赖（查询 owners）
    DateManager ..> AccountManager : 依赖（更新利息）
    TransactionManager ..> AccountManager : 依赖（查找账户）
    TransactionManager ..> UserManager : 依赖（验证持有人）
    TransactionManager ..> DateManager : 依赖（获取日期）
```
...
| 函数原型 | 功能说明 |
|----------|------|
| `bool deposit(int id, double amount, int pwd)` | 存款：验证账户+持有人+密码，调用 Account::deposit，返回是否成功 |
| `bool withdraw(int id, double amount, int pwd)` | 取款：验证账户+持有人+密码，调用 Account::withdraw，返回是否成功 |
| `bool transfer(int src, int dst, double amount, int pwd)` | 转账：验证双账户+持有人+密码，原子操作（先取后存），返回是否成功 |
| `bool fixedDeposit(int id, double amount, int months, int pwd)` | 定期存款：验证+类型为S，调用 SavingAccount::fixedDeposit，返回是否成功 |
| `bool fixedWithdraw(int id, double amount, int pwd)` | 定期部分提前支取：验证+类型为S，调用 SavingAccount::fixedWithdraw，返回是否成功 |
| `bool consume(int id, double amount, int pwd)` | 信用消费：验证+类型为C，调用 CreditAccount::consume，返回是否成功 |
| `bool cashAdvance(int id, double amount, int pwd)` | 信用取现：验证+类型为C，调用 CreditAccount::cashAdvance，返回是否成功 |
