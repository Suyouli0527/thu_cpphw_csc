...
### 8.12 信用账户 withdraw 与 cashAdvance 的统一

**问题背景**：信用账户的取现操作有两种看待方式——从银行角度是"取款"（与储蓄账户共用 WITHDRAW 命令），从信用角度是"透支取现"（CASH_ADVANCE 命令）。两者实质相同。

**统一方案**：CreditAccount 中 `withdraw()` 和 `cashAdvance()` 执行完全相同的透支取现逻辑（`withdraw` 内部就是取现代码，cashAdvance 在 AccountManager 层调用 `acc->withdraw()`）。Command 层两条命令都开放，方便用户通过 WITHDRAW（存取转账菜单）或 CASH_ADVANCE（信用账户菜单）两种路径操作。
