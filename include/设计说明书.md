...
SAVE 使用 `ofstream`（RAII 自动关闭），RESUME 使用 `ifstream` 逐行读取。使用 C++ 流式 I/O 而非 C 风格 `fopen`，文件打开失败输出 `0` 而非抛异常。
...
**正确性分析**：`getMaxDay()` 内部调用 `daysInMonth(year, month)`，自动处理闰年 2 月 = 29 天。每次循环 `day` 至少减少 28（2 月），所以循环次数最多 `n/28` 次。对于银行模拟系统的日期推进量（通常数百天），性能完全足够。
