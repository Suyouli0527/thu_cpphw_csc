...
**实现方案**：将 `BankSystem` 按职责拆分为 3 个 Manager 类：

- `AccountManager`：持有账户数据 + 系统日期，提供 CRUD、日期管理、交易操作、利息计算
- `UserManager`：持有用户数据和当前用户名，提供用户管理和权限校验
- `LogManager`：持有日志数据和操作结果标志

`BankSystem` 退化为 Facade 协调者，持有 3 个 Manager 实例，公开接口完全不变。关键设计决策：

1. **Manager 方法返回 bool**：各 Manager 操作方法返回成功/失败，由 BankSystem 统一处理日志记录和结果标志，Manager 层不依赖 LogManager，耦合最低
2. **指针依赖注入**：Manager 之间通过指针互相引用（AccountManager 内部持有 currentDate，日期变更时自动触发利息更新），在 BankSystem 构造函数中完成注入
...
5. **辅助方法消除重复**：BankSystem 引入 `logResult(bool)`、`requireAdmin()`、`requireAccount()` 私有辅助方法，交易委托从 6 行缩减为 1 行

### 8.11 账户密码与权限两级验证

**技术点**：用户密码（字符串）与账户密码（6 位数字）分离，所有交易操作需验证账户密码

**实现方案**：
- `User` 类存储字符串密码，`Account` 类存储 6 位数字密码（`accountPassword`）
- 开户时密码范围验证：`100000 <= pwd <= 999999`
- `requireAccount(id, pwd)` 辅助方法统一执行"查找账户 + 验证持有人 + 验证密码"三件套
- 用户密码修改独立于账户密码修改（`changeUserPassword` / `changeAccountPassword`）

### 8.12 共享账户多人共管机制

**技术点**：一个账户可被 2~5 人共同持有，管理员可动态添加/移除共有人

**实现方案**：
- `Account` 维护 `vector<string> owners` 列表，`User` 维护 `vector<int> accountIDs` 双向关联
- `addOwner`：检查上限 5 人、不重复，成功后同时更新 Account::owners 和 User::accountIDs
- `removeOwner`：检查至少保留 1 人，成功后双向清理
- `isShared` 标志位控制账户是否启用共享模式，管理员通过 `modifyShared` 切换

### 8.13 还款冲抵优先级算法

**技术点**：信用账户存款自动按优先级冲抵不同类型债务

**实现方案**：还款先补充余额到 0（还清透支），多余部分按成本从高到低冲抵：
1. 先冲取现债务 `cash_debt`（无免息期，日计息）
2. 再冲已过免息期的消费债务 `consume_debt_overdue`（按日计息中）
3. 最后冲免息期内消费债务 `consume_debt`（暂未计息）

这一顺序确保用户还款收益最大化（先还高息债务）。

### 8.14 信用透支判定与债务状态追踪

**技术点**：仅当余额为负时才产生透支债务，并非所有消费/取现都计为债务

**实现方案**：
- `withdraw()` / `consume()` 执行 `balance -= amount` 后检查 `balance < 0`
- 只有透支部分（`-balance`）才计入对应债务变量：取现计入 `cash_debt`，消费计入 `consume_debt`
- 用状态变量（`cash_debt`、`consume_debt`、`consume_debt_overdue`）代替交易流水重放，O(1) 查询债务余额
- 存款还款时余额恢复为正后，多余部分自动冲抵债务，变量同步更新

### 8.15 免息期动态计算与特殊日期处理

**技术点**：每月还款日可能超出当月实际天数，需安全处理边界

**实现方案**：
- 免息截止日为"消费所在下个月的还款日"：`graceDeadline = Date(下月, min(repaymentDay, 当月最大天数))`
- 每月推进时检查当天是否等于还款日，若是则将 `consume_debt` 全部移入 `consume_debt_overdue`
- 还款日范围限制为 1~28（所有月份均有 28 天），避免 29~31 日在 2 月等短月不存在
- `modifyRepaymentDay` 让用户可动态调整还款日
