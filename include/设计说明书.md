...
- **操作日志**：事件溯源式回滚与持久化恢复
- **命令行交互**：通过 stdin 逐行读取命令，`Command` 为纯静态工具类负责解析并分发
...
├──────────────────────────────────────────────────────────┤
│                  数据模型层                                  │
│    Account（储蓄/信用）  Date  User  InterestCalculator      │
└──────────────────────────────────────────────────────────┘
```

- **前端层**：`Command` 为纯静态工具类，通过 `istringstream` 解析 stdin 输入的命令字符串并分发给后端，解析失败输出 `0`。
...
- **数据模型层**：`Account` 类层次（`SavingAccount`、`CreditAccount`）封装账户数据和业务规则；`Date` 提供日期运算；`InterestCalculator` 为纯静态利率计算工具类，供模型层和服务层共用。
...
| `frozen` | `bool` | 是否已冻结。冻结后拒绝所有交易，但不影响利息计算 |
| `owners` | `vector<OwnerInfo>` | 共有人列表。每个条目含用户名、权限级别(FULL/RESTRICTED)、取款限额。开户者为FULL |
...
| `void addOwner(const string& userName, OwnerLevel level, double limit)` | 添加共有人：检查上限5人且不重复，指定权限级别(FULL/RESTRICTED)和取款限额 |
| `void removeOwner(const string& userName)` | 移除共有人：检查至少保留1人，按名称匹配 erase |
| `void modifyOwnerLimit(const string& userName, double newLimit)` | 修改RESTRICTED共有人的取款限额，仅FULL或管理员可操作 |
...
| 函数原型 | 功能说明 |
|----------|------|
| `static bool execute(BankSystem& system, const string& command, string& outAction)` | **核心静态函数**：将命令字符串通过 `istringstream` 解析为命令词+参数，按 `if/else if` 链匹配并调用对应 BankSystem 方法。成功时 `outAction` 接收命令词，解析失败返回 false。支持 35+ 种命令。被 BankSystem::replayCommands() 共用 |

#### HELP（查看命令列表）

**格式**：`HELP`

**输出**：列出全部支持的命令及其参数格式，分 10 类显示（账户、交易、定期、信用、日期、用户、共享、风控、密码、日志）。
...
**格式**：`ADD_OWNER <id> <userName> <FULL|RESTRICTED> [limit]`

**算法流程**：
1. **管理员权限检查**
2. 查找账户，验证存在且为共享账户（`isShared() == true`）
3. 验证目标用户存在
4. 解析级别字符串：`"FULL"` → `OwnerLevel::FULL`，`"RESTRICTED"` → `OwnerLevel::RESTRICTED`
5. 若为 RESTRICTED，额外解析 `withdrawLimit`（取款限额）
6. 调用 `Account::addOwner(userName, level, withdrawLimit)`：检查上限5人且不重复
7. 将 `id` 加入目标用户的 `accountIDs`
8. 成功则记录日志

#### MODIFY_OWNER_LIMIT（修改共有人的取款限额）

**格式**：`MODIFY_OWNER_LIMIT <id> <userName> <newLimit>`

**算法流程**：
1. 查找账户，验证存在且为共享账户
2. 验证当前用户为 FULL 级别共有人或管理员（非管理员命令）
3. 调用 `Account::modifyOwnerLimit(userName, newLimit)`：仅对 RESTRICTED 级别共有人生效

#### REMOVE_OWNER（移除共有人，FULL共有人或管理员）

**格式**：`REMOVE_OWNER <id> <userName>`

**算法流程**：
1. 查找账户，验证存在且为共享账户
2. 验证当前用户为 FULL 级别共有人或管理员
3. 调用 `Account::removeOwner(userName)`：检查至少保留1人
4. 将 `id` 从目标用户的 `accountIDs` 中移除
5. 成功则记录日志

#### WITHDRAW（RESTRICTED 限额检查）

信用账户取款时额外检查：若当前用户对该账户为 RESTRICTED 级别，`amount` 必须 ≤ `withdrawLimit`，否则操作失败。RESTRICTED 共有人仅能执行 DEPOSIT 和 WITHDRAW，TRANSFER/FIXED_DEPOSIT/FIXED_WITHDRAW/CONSUME/MODIFY 均被拒绝。

#### FIXED_DEPOSIT 自动转存参数

**格式**：`FIXED_DEPOSIT <id> <amount> <months> <pwd> [AUTO]`

末尾可追加 `AUTO` 关键字。设置 `autoRenew = true` 后，到期时不退回余额，而是用本金+利息按相同期限自动续存，`autoRenew` 持续有效直到提前支取。

#### FREEZE / UNFREEZE（冻结/解冻账户，仅管理员）

**格式**：`FREEZE <id>` / `UNFREEZE <id>`

**算法流程**：
1. **管理员权限检查**
2. 查找账户，验证存在
3. `FREEZE`：`acc->setFrozen(true)`；`UNFREEZE`：`acc->setFrozen(false)`
4. 冻结后所有交易操作被拒绝，利息计算不受影响
...
**提前支取惩罚**（`fixedWithdraw`）：查找首笔"未到期 + 未标记 + 金额 ≤ 本金"的定期存款。利息按活期 `savingRate` 重算：`利息 = 支取金额 × 活期年利率 × 持有天数 ÷ withdrawDate.daysInYear()`。标记 `partiallyWithdrawn = true` 后该笔不再参与后续匹配，防止同一笔被多次部分支取。

**自动转存**（`autoRenew`）：`FIXED_DEPOSIT` 命令末尾可追加 `AUTO` 参数。到期时若 `autoRenew == true`，不将本金退回余额，而是用 `本金 + 到期利息` 作为新本金，按相同期限自动续存。新到期日 = 原到期日 + fixedDays[months]。`autoRenew` 持续有效，每轮到期均自动转存。
...
**问题背景**：共享账户需区分共有人权限——开户人级别的共有人可进行所有操作，低级别共有人仅能存取款且有额度限制。

**数据结构**：`OwnerInfo` 结构体替代纯用户名存储：
...
**权限检查**：
- `ownsAccount()`：遍历 `o.name` 匹配，任何级别均视为持有人
- `withdraw()`：BankSystem 层检查 `getOwnerLevel(name) == RESTRICTED && amount > getWithdrawLimit(name)` → 拒绝
- `modifyOwnerLimit()`：仅 FULL 级别共有人或管理员可调用，无需管理员权限
- `removeOwner()`：FULL 级别共有人或管理员可调用
...
**技术点**：所有利率常量集中在 `InterestCalculator` 中作为 `static const` 成员，避免魔法数字散布各文件。模型层（Account）和服务层（AccountManager）均通过 `InterestCalculator::` 调用，不产生交叉依赖。
...
进阶版新增共享账户分级共管机制：共有人分两档——**全权限（FULL）** 和 **限取款（RESTRICTED）**。`OwnerInfo` 结构体存储用户名、权限级别、取款限额。

- `ADD_OWNER <id> <userName> <FULL|RESTRICTED> [limit]`：管理员添加共有人。RESTRICTED 需指定取款限额
- `MODIFY_OWNER_LIMIT <id> <userName> <newLimit>`：修改限额，仅 FULL 共有人或管理员可操作（非管理员命令）
- `REMOVE_OWNER <id> <userName>`：FULL 共有人或管理员可移除

取款时自动检查：RESTRICTED 共有人取款金额不得超过 `withdrawLimit`。FULL 共有人无限制。QUERY 输出显示共有人级别标识 `(F)`/`(R)`。
...
- **自动转存**：`FIXED_DEPOSIT` 末尾可追加 `AUTO` 参数。到期后用本利按相同期限自动续存，`autoRenew` 持续有效
...
Manager 之间零指针依赖，`BankSystem` 退化为 Facade 协调者，通过 `requireAdmin()`、`requireAccount(id)`、`requireAccount(id, pwd)`、`requireFullAccess(id)`、`logResult(bool)` 五个私有辅助方法统一调度，消除 40+ 处重复代码（权限守卫 12 处、账户验证 12 处、日志模式 12 处）。

### 9.7 账户冻结/解冻

基础版无账户风控机制。进阶版新增管理员冻结功能：

- `FREEZE <id>`：管理员冻结账户。冻结后所有交易（存款、取款、转账、定期、消费）均被拒绝，但不影响利息计算
- `UNFREEZE <id>`：管理员解冻账户，恢复正常交易

`Account` 新增 `frozen` 字段（默认 false），`AccountManager` 的 6 个交易方法开头统一检查 `acc->isFrozen()`。QUERY 输出增加冻结状态显示。

### 9.8 RESTRICTED 共有人存取款限制

进阶版共享账户的 RESTRICTED 共有人权限被严格限制为**仅存取款**：

- `DEPOSIT`、`WITHDRAW`：允许（取款需 ≤ withdrawLimit）
- `TRANSFER`、`FIXED_DEPOSIT`、`FIXED_WITHDRAW`、`CONSUME`、`MODIFY NAME`：拒绝
- `requireFullAccess(id)` 封装 FULL 级别检查，管理员和 FULL 共有人直接放行
