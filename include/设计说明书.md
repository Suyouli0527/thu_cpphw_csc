...
classDiagram
    ...
    CreditAccount *-- CreditTransaction : 组合（内含多条交易流水）
```
...
classDiagram
    ...
    CreditAccount *-- CreditTransaction : 组合（内含多条流水）
    Account *-- Date : 组合（openDate, lastInterestDate）
...
| `void deposit/withdraw/transfer/fixedDeposit/fixedWithdraw/consume/cashAdvance(...)` | transactionMgr → 成功时 logMgr 记录 |
...
| 函数原型 | 功能说明 |
|----------|------|
| `Account* validateAndGet(int id, int pwd)` | **私有辅助方法**：统一执行账户验证三件套（findAccount + ownsAccount + verifyPassword），验证通过返回 Account*，失败返回 nullptr。消除 7 个交易方法中的重复验证代码 |
| `bool deposit(int id, double amount, int pwd)` | 存款：调用 `validateAndGet()` 验证，调用 Account::deposit，返回是否成功 |
| `bool withdraw(int id, double amount, int pwd)` | 取款：调用 `validateAndGet()` 验证，调用 Account::withdraw，返回是否成功 |
...
| `bool consume(int id, double amount, int pwd)` | 信用消费：调用 `validateAndGet()` 验证+类型为C，调用 CreditAccount::consume，返回是否成功 |
| `bool cashAdvance(int id, double amount, int pwd)` | 信用取现：调用 `validateAndGet()` 验证+类型为C，调用 CreditAccount::cashAdvance，返回是否成功 |
