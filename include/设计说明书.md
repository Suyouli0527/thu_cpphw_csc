...
| `currentMode` | `enum Mode` | 当前模式：`NONE`/`ADMIN`/`USER`/`CARD`。构造时初始化为 `NONE`，进入模式循环前赋为对应值，退出后重置为 `NONE`。供各 `collectXxx()` 函数判断是否需调用 `injectCardId` 注入卡号 |
...
| `void updateAllAccountsInterest(const Date& newDate)` | 遍历全部账户，依次调用储蓄账户的 `updateInterest`、`updateFixedDeposits`、`updateWealthProducts` 和信用账户的 `updateCreditInterest`。日期推进时触发 |
...
| `void printAccountDetail(int id) const` | 多行详细输出账户全部信息（基本信息、定期明细、基金持仓含当前净值/盈亏、理财持仓含预期收益） |
...
| `void closeAccount(int id, int accountPassword)` | 销户：验证存在+持有人+密码。余额须为0，储蓄账户须无定期/基金/理财持仓。调用 `removeAccount` 清理，记录日志 |
...
| 函数原型 | 功能说明 |
|----------|------|
| `void buyFund(int id, int fundIndex, double amount, int accountPassword)` | 购买基金：验证存在+持有人+密码+类型为'S'，调用 `SavingAccount::buyFund(currentDate, fundIndex, amount)`，记录日志 |
| `void sellFund(int id, int holdingIndex, int accountPassword)` | 赎回基金：验证存在+持有人+密码+类型为'S'，调用 `SavingAccount::sellFund(holdingIndex, currentDate)`，记录日志 |
| `void buyWealthProduct(int id, int productIndex, double amount, int accountPassword)` | 购买理财产品：验证存在+持有人+密码+类型为'S'，调用 `SavingAccount::buyWealthProduct(currentDate, productIndex, amount)`，记录日志 |
...
| `void addDays(int days)` | 推进日期（仅管理员）：验证权限+days>0，调用 `Date::addDays(days)`，然后触发 `updateAllAccountsInterest(currentDate)`（活期结算+定期到期+理财到期），记录日志 |
...
| `Account* findAccountForTest(int id) const` | 测试用接口，暴露 `findAccount` 给单元测试 |
...
| 函数原型 | 功能说明 |
|----------|------|
| `void run()` | 主入口：显示主菜单，分发到三种模式循环。进入模式前设置 `currentMode`（`ADMIN`/`USER`/`CARD`），退出模式后重置为 `NONE`，确保 `collectXxx()` 函数正确判断是否调用 `injectCardId` |
...
操作路径: 插卡模式 → [1] 存取转账 → [1] 存款
输入存款金额: 10000
输入账户密码: 123456
输出: [存款成功]

操作路径: 插卡模式 → [1] 存取转账 → [2] 取款
输入取款金额: 2000
输入账户密码: 123456
输出: [取款成功]

操作路径: 插卡模式 → [1] 存取转账 → [3] 转账
输入转入账户ID: 3001
输入转账金额: 1000
输入账户密码: 123456
输出: [转账成功]
...
操作路径: 插卡模式 → [4] 投资理财 → [2] 基金赎回
输入要赎回的持仓编号: 0
输入账户密码: 123456
输出: [基金赎回成功]

操作路径: 插卡模式 → [4] 投资理财 → [3] 理财产品购买
系统显示:
  可选理财产品:
    [0] 短期理财A 期限30天 年化2.5%
    [1] 短期理财B 期限90天 年化3%
...
输出:
  ...
  基金持仓: 0只
  理财持仓: 1笔
    [0] 短期理财A 本金:5000.00 预期收益:10.27 购买:2024-1-1 到期:2024-1-31
  [操作成功]

操作路径: 插卡模式 → [6] 查询账户 → [2] 查询所有账户
输出: 1001 S mysavings 83011.71 SHARED admin zhangsan 2 12 7000.00 2024-1-1 2024-12-31 1 6 5000.00 2024-1-1 2024-6-29 0 0 1 0 5000.00 2024-1-1 2024-1-31 NOT_SETTLED
      2001 C creditcard 0.00 NOT_SHARED 10000.00 15 0.00 0.00
      3001 S second 19602.34 NOT_SHARED 0 0 0
      [操作成功]
```
...
```
操作路径: 插卡模式 → [5] 账户信息修改 → [1] 修改账户名
输入新账户名称: newsavings
输入账户密码: 123456
输出: [修改成功]

操作路径: 插卡模式 → [5] 账户信息修改 → [2] 修改账户密码
输入旧密码: 123456
输入新密码: 999888
输出: [账户密码修改成功]
...
### 7.11 插卡模式 — 信用账户操作（账户 2001）

```
操作路径: 主菜单 → [3] 插卡操作 → 输入账户ID: 2001, 密码: 654321
输出: ===== 插卡模式 (账户 #2001) =====

操作路径: 插卡模式 → [3] 信用账户 → [1] 消费
输入消费金额: 3000
输入账户密码: 654321
输出: [消费成功]

操作路径: 插卡模式 → [3] 信用账户 → [2] 取现
输入取现金额: 1000
输入账户密码: 654321
输出: [取现成功]

操作路径: 插卡模式 → [1] 存取转账 → [1] 存款（还款）
输入存款金额: 2000
输入账户密码: 654321
输出: [存款成功]
...
输出: 1001 S newsavings 83011.71 SHARED admin zhangsan 2 12 7000.00 2024-1-1 2024-12-31 1 6 5000.00 2024-1-1 2024-6-29 0 0 1 0 5000.00 2024-1-1 2024-1-31 NOT_SETTLED
      [操作成功]

操作路径: 普通用户模式 → [8] 查询当前用户
输出: zhangsan
      [操作成功]
...
16 BUY_FUND 1001 0 5000 123456
17 SELL_FUND 1001 0 123456
18 BUY_WP 1001 0 5000 123456
19 MODIFY NAME 1001 newsavings 123456
20 MODIFY_ACCOUNTPASSWORD 1001 123456 999888
21 CONSUME 2001 3000 654321
22 CASH_ADVANCE 2001 1000 654321
23 DEPOSIT 2001 2000 654321
24 QUERY 2001 654321
25 MODIFY_USERPASSWORD abc123 xyz789
26 MODIFY_USERPASSWORD abc123 xyz789
27 ADD_DAY 365
28 MODIFY CREDIT 2001 20000 654321
29 REMOVE_OWNER 1001 zhangsan
30 DELETE_USER zhangsan
      [操作成功]
```

### 7.18 管理员模式 — 回滚（ROLLBACK）
