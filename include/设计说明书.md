...
### 3.3 OwnerInfo 结构体与 OwnerLevel 枚举
...
## 第七章 主要技术难点及实现方案/算法设计

> 本章详细阐述系统中的核心算法与设计决策，是系统实现的技术亮点。

### 7.1 日期正交化算法（totalDays 编码与公式推导）
...
**设计收益**：
- 时间复杂度 O(天数) 而非 O(交易数 × 天数)
- 空间复杂度 O(1)，无需存储交易流水
- 状态机模型清晰，易于理解和维护

### 7.4 定期存款：固定利率查表 + 到期自动结算 + 提前支取惩罚 + 自动转存
...
三个平行数组 `fixedMonths`/`fixedRates`/`fixedDays` 以相同下标关联，`getFixedRate` 在 `fixedMonths` 中线性查找匹配月数，返回对应利率。时间复杂度 O(6) = O(1)。
...
- 时间开销：O(n) 重放，对于日志量不大的银行模拟系统完全可接受
- 通用性：SAVE/RESUME 与 ROLLBACK 共享同一套 `replayCommands` 机制

### 7.6 Facade 模式 + Manager 零耦合架构
...
**运行方式**：程序启动后从 `stdin` 逐行读取命令，通过 `stdout` 输出结果。支持管道输入：

```bash
./bank.exe < commands.txt
```

**测试流程**：

1. 用户管理：`SWITCH admin admin` → `CREATE_USER zhangsan abc123`
2. 开户：`OPEN 1001 S mysavings 50000 123456 0` → `OPEN 2001 C creditcard 10000 15 654321 0`
3. 存取转账：`DEPOSIT 1001 10000 123456` → `TRANSFER 1001 3001 1000 123456`
4. 定期存款：`FIXED_DEPOSIT 1001 10000 12 123456` → 推进日期 → 验证到期自动结算
5. 信用操作：`CONSUME 2001 3000 654321` → 推进日期 → 验证免息期/透支计息
6. 共享管理：`MODIFY SHARED 1001 ys` → `ADD_OWNER 1001 zhangsan FULL`
7. 回滚：`ROLLBACK 5` → 验证状态恢复
8. 持久化：`SAVE backup.log` → 重启 → `RESUME backup.log` → 验证一致
