...
classDiagram
    ...
    class Account {
        ...
        +Account(int id, char type, const string& name, double balance, const Date& openDate, const string& pwd, bool isShared)
        ...
        +bool verifyAccountPassword(const string& pwd) const
        +bool changeAccountPassword(const string& oldPwd, const string& newPwd)
    ...
    class SavingAccount {
        ...
        +SavingAccount(int id, char type, const string& name, double balance, const Date& openDate, const string& pwd, bool isShared)
    ...
    class CreditAccount {
        ...
        +CreditAccount(int id, char type, const string& name, double creditAmount, int repDay, const Date& openDate, const string& pwd, bool isShared)
        +bool deposit(const Date& date, double amount) override
        +bool withdraw(const Date& date, double amount) override
    ...
    class AccountManager {
        ...
        +void reset()
        +bool deposit(int id, double amount, const string& accountPassword)
        +bool withdraw(int id, double amount, const string& accountPassword)
        +bool transfer(int srcId, int dstId, double amount, const string& srcAccountPassword)
        +bool fixedDeposit(int id, double amount, int months, const string& accountPassword, bool autoRenew)
        +bool fixedWithdraw(int id, double amount, const string& accountPassword)
        +bool consume(int id, double amount, const string& accountPassword)
    }
    ...
    class BankSystem {
        ...
        -bool requireAccount(int id, const string& accountPassword) const
        -bool requireFullAccess(int id) const
        -void logResult(bool ok)
        +BankSystem()
        +~BankSystem()
        +void openAccount(int id, char type, const string& accountName, double balance, int repaymentDay, const string& accountPassword, bool shared)
        +void closeAccount(int id, const string& accountPassword)
        ...
        +void modifyName(int id, const string& username, const string& accountPassword)
        +void modifyCredit(int id, double newCredit, const string& accountPassword)
        ...
        +void changeAccountPassword(int id, const string& oldPassword, const string& newPassword)
        +void query(int id, const string& accountPassword) const
        +void queryAllAccounts() const
        +void deposit(int id, double amount, const string& accountPassword)
        +void withdraw(int id, double amount, const string& accountPassword)
        +void transfer(int srcId, int dstId, double amount, const string& srcAccountPassword)
        +void fixedDeposit(int id, double amount, int months, const string& accountPassword, bool autoRenew)
        +void fixedWithdraw(int id, double amount, const string& accountPassword)
        +void setAutoRenew(int id, int index, bool autoRenew, const string& accountPassword)
        +void consume(int id, double amount, const string& accountPassword)
        +void showDate() const
...
**提前支取惩罚**（`fixedWithdraw`）：查找首笔"未到期 + 未标记 + 本金 ≥ amount"的定期存款。利息按活期 `savingRate` 重算而非定期利率：
...
**自动转存**（`autoRenew`）：`FIXED_DEPOSIT` 命令末尾可追加 `AUTO` 参数。到期时若 `autoRenew == true`，不退回余额，而是用 `本金 + 到期利息` 作为新本金，按相同期限自动续存。`autoRenew` **持续有效**——每轮到期均自动转存，直到提前支取或通过 `SET_AUTORENEW` 命令关闭。`SET_AUTORENEW <id> <index> <ON|OFF> <pwd>` 可在到期前随时修改任意一笔定期存款的自动转存开关。
...
- **账户密码**（`Account::accountPassword`）：6 位纯数字字符串（000000~999999）。采用 `std::string` 类型存储，通过 `isValidPassword` 静态方法逐字符验证（`c < '0' || c > '9'`），支持前导零。所有账户操作（存款、取款、转账、查询、修改信息）均需验证。`MODIFY_ACCOUNTPASSWORD` 修改

**安全优势**：
- 即使用户被冒用登录，攻击者仍需知道账户密码才能操作账户
- 不同账户有不同密码，降低单点泄露风险
- 账户密码限制为 6 位纯数字字符串，避免特殊字符解析问题，同时支持前导零（如 000123）
...
对于日期和天数等必须为正整数的参数，使用 `isPositiveInt()` 辅助函数**先验证为纯数字字符串再转换**
...
| `totalDays` | `int` | 自公元 1 年 1 月 1 日起的累计天数。**日期运算的核心字段**——所有日期比较、求差运算退化为整数操作。通过 `getTotalDays()` 公开只读访问 |
...
| `debtRate` | `static const double` | 透支日利率 `0.0005`（年化约 18.25%）。 |
...
| `lastInterestDate` | `Date` | 上次计息截止日期。`updateInterest()` 从此日期逐日推进到目标日期，完成后更新为此日期。|
| `interest` | `double` | 当月累计未结利息。每日计息时累加到此字段；每月 1 日执行 `settleMonthlyInterest()` 将其结入 `balance`（月复利），然后清零 |
| `accountPassword` | `string` | 6 位数字字符串账户密码（000000~999999）。使用 `isValidPassword` 静态方法逐字符验证，确保恰好 6 位纯数字。所有账户操作（存款、取款、转账、查询等）均需验证此密码 |
...
| 函数原型 | 功能说明 |
|----------|------|
| `Account(int id, char type, const string& name, double balance, const Date& openDate, const string& pwd, bool isShared)` | 构造函数。初始化所有字段，`lastInterestDate = openDate`，`interest = 0`，`frozen = false`。密码通过 `isValidPassword` 验证 |
...
| `static bool isValidPassword(const string& pwd)` | 验证密码格式：长度必须为 6，逐字符检查是否均为数字（'0'~'9'），支持 000000~999999 |
| `bool verifyAccountPassword(const string& pwd) const` | 验证账户密码：直接比较 `accountPassword == pwd` |
| `bool changeAccountPassword(const string& oldPwd, const string& newPwd)` | 修改密码：验证旧密码匹配且新密码通过 `isValidPassword` 验证（恰好 6 位纯数字，000000~999999） |
...
| `bool setAutoRenew(int index, bool autoRenew)` | 修改指定定期存款的自动转存开关：验证 index 合法后设置 `fixedDeposits[index].autoRenew`。可在到期前随时开启或关闭 |
...
| `bool requireAccount(int id, const string& accountPassword) const` | 在上述基础上增加 `verifyAccountPassword` 校验。消除 7 处重复 |
...
| `void closeAccount(int id, const string& pwd)` | **BankSystem 自行协调**：验证余额为 0 且无定期 → 清理所有 owner 的关联 → `accountMgr.removeAccount` |
...
| `void setAutoRenew(int id, int index, bool autoRenew, const string& accountPassword)` | 验证账户 + 密码 + 类型为 S，委托 `SavingAccount::setAutoRenew` |
...
7. 验证 `accountPassword` 为 6 位纯数字字符串（通过 `Account::isValidPassword` 逐字符验证，支持 000000~999999）
...
#### SET_AUTORENEW（修改自动转存开关）

**格式**：`SET_AUTORENEW <id> <index> <ON|OFF> <accountPassword>`

**算法流程**：
1. 验证账户存在、当前用户为持有人、账户密码正确
2. 验证账户类型为 `'S'`
3. 验证 index 在定期存款列表范围内
4. 设置 `fixedDeposits[index].autoRenew` 为 ON（true）或 OFF（false）
5. 成功则记录日志
