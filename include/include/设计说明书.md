...
- **账户体系**：储蓄账户（活期 + 定期存款）、信用账户（透支消费 + 透支取现，债务状态追踪）
...
│  ┌──────────────────────────────────────────────────────┐ │
│  │ AccountManager │ UserManager │ DateManager           │ │
│  │ TransactionManager        │ LogManager               │ │
│  └──────────────────────────────────────────────────────┘ │
├──────────────────────────────────────────────────────────┤
│                  数据模型层                                  │
│  Account（储蓄/信用）  Date  InterestCalculator  User      │
└──────────────────────────────────────────────────────────┘
```

- **前端层**：`BankUI` 负责二级引导式菜单，收集用户输入并构造命令字符串；`Command` 为纯静态工具类，负责解析命令文本并分发给后端。前端不包含任何业务逻辑。
- **后端层**：`BankSystem` 作为 Facade 协调者，负责权限验证、日志记录、跨模块协调（开户、销户、共享管理、回滚）。内部将职责委托给 5 个 Manager：`AccountManager`（账户 CRUD 与打印）、`UserManager`（用户管理与权限）、`DateManager`（日期推进与利息更新）、`TransactionManager`（存取款、转账、定期、信用交易）、`LogManager`（日志记录与操作结果）。所有操作的成功/失败结果通过 `LogManager::lastResult()` 传递回前端。
...
#### 2.1.2 InterestCalculator 类（纯静态工具类）
...
classDiagram
    ...
    Account ..> InterestCalculator : 依赖（调用静态利率方法）
    Account *-- Date : 组合（openDate, lastInterestDate）
```

#### 2.1.4 SavingAccount 类
...
classDiagram
    ...
    Account <|-- SavingAccount : 继承（实现 deposit/withdraw）
    Account <|-- CreditAccount : 继承（实现 deposit/withdraw + consume）
    SavingAccount *-- FixedDeposit : 组合（内含多笔定期）
    Account *-- Date : 组合（openDate, lastInterestDate）
    Account ..> InterestCalculator : 依赖（调用静态利率计算）
```

### 2.2 Manager 层

#### 2.2.1 AccountManager 类
...
classDiagram
    ...
    AccountManager *-- Account : 组合（拥有所有权，析构时 delete）
```
...
classDiagram
    ...
    UserManager *-- User : 组合（按值存储所有用户）
    UserManager ..> AccountManager : 依赖（通过指针调用 findAccount 查询 owners）
```
...
classDiagram
    ...
    DateManager *-- Date : 组合（持有系统当前日期）
    DateManager ..> AccountManager : 依赖（日期推进时遍历账户更新利息）
```

#### 2.2.4 TransactionManager 类
...
classDiagram
    ...
    class TransactionManager {
        ...
        -UserManager* userMgr
        -DateManager* dateMgr
        -Account* validateAndGet(int id, int pwd)
    ...
    TransactionManager ..> AccountManager : 依赖（查找账户、执行交易）
    TransactionManager ..> UserManager : 依赖（验证持有人身份）
    TransactionManager ..> DateManager : 依赖（获取当前日期）
```
...
classDiagram
    ...
    UserManager ..> AccountManager : 依赖（查询 owners）
    DateManager ..> AccountManager : 依赖（日期推进更新利息）
    TransactionManager ..> AccountManager : 依赖（查找账户）
    TransactionManager ..> UserManager : 依赖（验证持有人）
    TransactionManager ..> DateManager : 依赖（获取当前日期）
    LogManager ..> None : 无外部依赖（纯日志管理）
```

### 2.3 系统架构层

#### 2.3.1 BankSystem 类（Facade 协调者）
...
classDiagram
    ...
    BankSystem *-- AccountManager : 组合（内含实例）
    BankSystem *-- UserManager : 组合（内含实例）
    BankSystem *-- DateManager : 组合（内含实例）
    BankSystem *-- LogManager : 组合（内含实例）
    BankSystem *-- TransactionManager : 组合（内含实例）
```

#### 2.3.2 Command 类（纯静态工具类）
...
classDiagram
    ...
    Command ..> BankSystem : 依赖（解析命令并调用 BankSystem 方法）
```
...
classDiagram
    ...
    BankUI o-- BankSystem : 聚合（引用外部 BankSystem 对象，不拥有）
```
...
classDiagram
    ...
    BankUI o-- BankSystem : 聚合（引用，不拥有）
    Command ..> BankSystem : 依赖（解析命令并调用）
    ...
    AccountManager *-- Account : 组合（拥有所有权，析构时 delete）
    UserManager *-- User : 组合（按值存储）
    DateManager *-- Date : 组合（持有系统当前日期）

    UserManager ..> AccountManager : 依赖（查询账户 owners 列表）
    DateManager ..> AccountManager : 依赖（日期推进时遍历账户更新利息）
    TransactionManager ..> AccountManager : 依赖（查找账户、执行交易）
    TransactionManager ..> UserManager : 依赖（验证持有人身份）
    TransactionManager ..> DateManager : 依赖（获取当前日期）
    ...
    Account <|-- SavingAccount : 继承（实现 deposit/withdraw）
    Account <|-- CreditAccount : 继承（实现 deposit/withdraw）
    SavingAccount *-- FixedDeposit : 组合（内含多笔定期存款）
    Account *-- Date : 组合（openDate, lastInterestDate）
    Account ..> InterestCalculator : 依赖（调用静态利率计算）
```
...
### 3.7 BankSystem 类（Facade 协调者）

| 成员 | 类型 | 说明 |
|------|------|------|
| `accountMgr` | `AccountManager` | 账户管理器，负责账户 CRUD、打印 |
| `userMgr` | `UserManager` | 用户管理器，负责用户管理、权限校验 |
| `dateMgr` | `DateManager` | 日期管理器，负责日期推进、利息更新 |
| `logMgr` | `LogManager` | 日志管理器，负责日志记录、操作结果标志 |
| `transactionMgr` | `TransactionManager` | 交易管理器，负责存取款、转账、定期、信用交易 |

### 3.8 AccountManager 类
...
| `accountMgr` | `AccountManager*` | 指向 AccountManager 的指针，用于日期推进时更新利息 |

### 3.11 TransactionManager 类

无自有数据成员，仅持有 AccountManager*、UserManager*、DateManager* 指针用于委托操作。
...
| `void updateCreditInterest(const Date& targetDate)` | 逐日透支计息：从 lastInterestUpdate 到 targetDate 逐天遍历。每日若为还款日则将 `consume_debt` 移入 `consume_debt_overdue`。日利息 = `cash_debt * debtRate` + `consume_debt_overdue * debtRate` |
...
| `double getConsumeDebtGrace() const` | 返回 `consume_debt`（免息期内消费债务） |
| `double getConsumeDebtOverdue() const` | 返回 `consume_debt_overdue`（已过免息期消费债务，按日计息中） |
| `bool modifyCredit(double newCredit)` | 修改信用额度，直接赋值 |
| `bool modifyRepaymentDay(int newDay)` | 修改还款日（1~28），处理特殊日期：若某月天数 < 还款日则取当月最大天数 |
| `bool isWithinGracePeriod(const Date& consumeDate, const Date& checkDate) const` | 私有：判断 checkDate 是否在 consumeDate 的免息期内（截止日为 consumeDate 下个月的还款日） |
...
| `void logResult(bool ok)` | 日志记录辅助方法：`ok` 为 true 时调用 `logMgr.printSuccess()` + `logMgr.recordLog()`，false 时调用 `logMgr.printFailure()`。消除 BankSystem 中 12 处重复的日志记录模式 |
| `bool requireAdmin() const` | 管理员权限守卫：检查 `userMgr.isAdmin()`，非管理员时调用 `logMgr.printFailure()` 并返回 false。消除 BankSystem 中 12 处重复的权限检查模式 |
...
- **简单委托**：`deposit`/`withdraw`/`transfer`/`fixedDeposit`/`fixedWithdraw`/`consume` 委托给 `transactionMgr`，返回 bool 决定日志记录
...
| `void deposit/withdraw/transfer/fixedDeposit/fixedWithdraw/consume(...)` | transactionMgr → 成功时 logMgr 记录 |
...
| 函数原型 | 功能说明 |
|----------|------|
| `void showDate() const` | 格式化输出当前日期 |
| `bool addDays(int days)` | 推进日期，更新全账户利息（S: updateInterest + updateFixedDeposits，C: updateCreditInterest），返回是否成功 |
| `bool setDate(int y, int m, int d)` | 设置日期（拒绝倒退），更新全账户利息，返回是否成功 |
| `const Date& getCurrentDate() const` | 返回当前日期的只读引用 |
| `void reset()` | 重置日期到 1970-01-01 |

### 4.11 TransactionManager 类

| 函数原型 | 功能说明 |
|----------|------|
| `Account* validateAndGet(int id, int pwd)` | **私有辅助方法**：统一执行账户验证三件套（findAccount + ownsAccount + verifyPassword），验证通过返回 Account*，失败返回 nullptr |
| `bool deposit(int id, double amount, int pwd)` | 存款/还款：调用 `validateAndGet()` 验证，调用 Account::deposit，返回是否成功 |
| `bool withdraw(int id, double amount, int pwd)` | 取款/取现：调用 `validateAndGet()` 验证，调用 Account::withdraw（信用账户即为透支取现），返回是否成功 |
| `bool transfer(int src, int dst, double amount, int pwd)` | 转账：调用 `validateAndGet()` 验证源账户，查找目标账户，原子操作（先取后存），返回是否成功 |
| `bool fixedDeposit(int id, double amount, int months, int pwd)` | 定期存款：调用 `validateAndGet()` 验证+类型为S，调用 SavingAccount::fixedDeposit，返回是否成功 |
| `bool fixedWithdraw(int id, double amount, int pwd)` | 定期部分提前支取：调用 `validateAndGet()` 验证+类型为S，调用 SavingAccount::fixedWithdraw，返回是否成功 |
| `bool consume(int id, double amount, int pwd)` | 透支消费：调用 `validateAndGet()` 验证+类型为C，调用 CreditAccount::consume，返回是否成功 |
...
5. 信用账户：透支取现，`balance -= amount`；若余额为负，透支部分计入 `cash_debt`
6. 成功则记录日志
...
4. `balance -= amount`。若余额为负，透支部分计入 `consume_debt`
5. 透支消费享免息期至次月还款日，逾期未还部分按日利率计息
6. 成功则记录日志

#### WITHDRAW（信用账户取现）

信用账户的 `WITHDRAW` 命令等同于透支取现，流程同普通取款但仅透支部分计入债务：
1. `balance -= amount`。若余额为负，透支部分计入 `cash_debt`
2. 取现债务从次日起按日利率计息，无免息期
...
操作路径: 管理员模式 → [2] 存取转账 → [2] 取款（信用账户即为取现）
输入账户ID: 2001
输入取现金额: 1000
输入账户密码: 654321
输出: [取款成功]
...
**技术点**：消费有免息期，取现无免息期；透支部分才计入债务；还款优先冲抵取现

**实现方案**：
- **债务状态变量**：`cash_debt`（取现债务）、`consume_debt`（免息期内消费债务）、`consume_debt_overdue`（已过免息期消费债务）
- **透支判定**：`withdraw()`/`consume()` 仅在余额为负时将透支部分计入对应债务变量
- **还款冲抵**：`deposit()` 余额为正时依次冲抵 `cash_debt` → `consume_debt_overdue` → `consume_debt`
- **计息逻辑**：每日若为还款日则将 `consume_debt` 移入 `consume_debt_overdue`；日利息 = `cash_debt * debtRate + consume_debt_overdue * debtRate`
- **特殊日期处理**：当月天数 < 还款日时取当月最大天数
...
### 8.10 Facade 模式与模块化拆分

**技术点**：将单体类拆分为独立 Manager，使用 Facade 模式保持接口不变

**实现方案**：将 `BankSystem` 按职责拆分为 5 个 Manager 类：

- `AccountManager`：持有账户数据，提供 CRUD 和打印
- `UserManager`：持有用户数据和当前用户名，提供用户管理和权限校验
- `DateManager`：持有系统日期，负责日期推进和利息更新
- `TransactionManager`：无状态，提供金融交易操作
- `LogManager`：持有日志数据和操作结果标志

`BankSystem` 退化为 Facade 协调者，持有 5 个 Manager 实例，公开接口完全不变。关键设计决策：

1. **Manager 方法返回 bool**：各 Manager 操作方法返回成功/失败，由 BankSystem 统一处理日志记录和结果标志，Manager 层不依赖 LogManager，耦合最低
2. **指针依赖注入**：Manager 之间通过指针互相引用（如 DateManager 持有 AccountManager* 来更新利息），在 BankSystem 构造函数中完成注入
3. **权限检查留在 Facade 层**：`isAdmin()` 等权限判断在 BankSystem 中进行，Manager 层不做权限判断
4. **跨模块协调留在 Facade**：`openAccount()`、`closeAccount()`、`addOwner()`、`removeOwner()`、`rollback()` 需要协调多个 Manager，留在 BankSystem 中
5. **辅助方法消除代码重复**：`BankSystem` 提取 `logResult(bool)` 和 `requireAdmin()` 两个私有辅助方法，`TransactionManager` 提取 `validateAndGet(int, int)` 私有辅助方法，分别消除了 12 处日志记录、12 处管理员权限守卫、7 处账户验证的重复代码
